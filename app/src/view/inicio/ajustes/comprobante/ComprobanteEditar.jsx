...
import { comboTipoComprobante, editComprobante, getIdComprobante } from '../../../../network/rest/principal.network';
...
class ComporbanteEditar extends CustomComponent {
    ...
    handleGuardar = () => {
        if (!isText(this.state.idTipoComprobante)) {
            alertWarning("Comprobante", "Seleccione el tipo de comprobante.", () => this.refTipo.current.focus());
            return;
        }

        if (!isText(this.state.nombre)) {
            alertWarning("Comprobante", "Ingrese el nombre de comprobante.", () => this.refNombre.current.focus());
            return;
        }

        if (!isText(this.state.serie)) {
            alertWarning("Comprobante", "Ingrese la serie del comprobante.", () => this.refSerie.current.focus());
            return;
        }

        if (!isNumeric(this.state.numeracion)) {
            alertWarning("Comprobante", "Ingrese la numeración.", () => this.refNumeracion.current.focus());
            return;
        }
                            ...
                            >
                                <option value="">- Seleccione -</option>
                                {
                                    this.state.tipoComprobante.map((item, index) => (
                                        <option key={index} value={item.idTipoComprobante}>{item.nombre}</option>
                                    ))
                                }
                            </select>
                        </div>
