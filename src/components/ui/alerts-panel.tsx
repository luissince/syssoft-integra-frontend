  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {alerts.length === 0 ? (
          <div className="flex h-20 items-center justify-center text-muted-foreground">No hay alertas pendientes</div>
        ) : (
          alerts.map((alert, index) => {
            const Icon = alert.icon
            return (
              <Alert key={index} className={getSeverityStyles(alert.severity)}>
                <Icon className={`h-4 w-4 ${getSeverityIcon(alert.severity)}`} />
