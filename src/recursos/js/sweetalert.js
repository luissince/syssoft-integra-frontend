const Swal = () => {
    ...
    var setParameters = function setParameters(params) {
        ...
        // border radius
        var popupBorderRadiues =
            params.borderRadius === defaultParams.borderRadius && params.toast
                ? "0.5rem"
                : params.borderRadius;
        popup.style.borderRadius = popupBorderRadiues;
    ...
    // Variables para las clases de estilo y tipos de iconos
    const dialogClass = swalClasses.popup;
    const progressStepsClass = swalClasses.progresssteps;
    const errorIconClass = `${swalClasses.icon} ${iconTypes.error}`;
    const questionIconClass = `${swalClasses.icon} ${iconTypes.question}`;
    const warningIconClass = `${swalClasses.icon} ${iconTypes.warning}`;
    const infoIconClass = `${swalClasses.icon} ${iconTypes.info}`;
    const successIconClass = swalClasses.icon + " " + iconTypes.success;
    const successCircularLineLeft = "swal2-success-circular-line-left";
    const successLineTip = "swal2-success-line-tip";
    const successLineLong = "swal2-success-line-long";
    const successRing = "swal2-success-ring";
    const successFix = "swal2-success-fix";
    const successCircularLineRight = "swal2-success-circular-line-right";
    ...
    const fileInputClass = swalClasses.file;
    const rangeClass = swalClasses.range;
    const selectClass = swalClasses.select;
    const radioClass = swalClasses.radio;
    const checkboxClass = swalClasses.checkbox;
    const textareaClass = swalClasses.textarea;
    const validationErrorClass = swalClasses.validationerror;
    const buttonWrapperClass = swalClasses.buttonswrapper;
    const confirmButtonClass = swalClasses.confirm;
    const cancelButtonClass = swalClasses.cancel;
    const closeButtonClass = swalClasses.close;

    // Crear el fragmento de HTML
    const sweetHTML = `
  <div role="dialog" aria-modal="true" aria-labelledby="${swalClasses.title}" aria-describedby="${swalClasses.content}" class="${dialogClass}" tabindex="-1">
  
    <div class="swal2-header">
        SysSoft Integra
    </div>

    <ul class="${progressStepsClass}"></ul>
    
    <div class="${contentWrapperClass}">
        <div class="${errorIconClass}">
            <span class="swal2-x-mark"><span class="swal2-x-mark-line-left"></span><span class="swal2-x-mark-line-right"></span></span>
        </div>
        <div class="${questionIconClass}">?</div>
        <div class="${warningIconClass}">!</div>
        <div class="${infoIconClass}">i</div>
        <div class="${successIconClass}">
            <div class="${successCircularLineLeft}"></div>
            <span class="${successLineTip}"></span> <span class="${successLineLong}"></span>
            <div class="${successRing}"></div> <div class="${successFix}"></div>
            <div class="${successCircularLineRight}"></div>
        </div>
        <img class="${imageClass}" />
        <div class="${contentdetailClass}">
            <h2 class="${titleClass}" id="${swalClasses.title}"></h2>
            <div id="${contentClass}" class="${contentClass}"></div>
        </div>
    </div>
    ...
    <label for="${checkboxClass}" class="${checkboxClass}">
      <input type="checkbox" />
    </label>

    <textarea class="${textareaClass}"></textarea>

    <div class="${validationErrorClass}" id="${validationErrorClass}"></div>

    <div class="${buttonWrapperClass}">
      <button type="button" class="${confirmButtonClass}">OK</button>
      <button type="button" class="${cancelButtonClass}">Cancel</button>
    </div>

    <button type="button" class="${closeButtonClass}">\xD7</button>
  </div>
`.replace(/(^|\n)\s*/g, "");
