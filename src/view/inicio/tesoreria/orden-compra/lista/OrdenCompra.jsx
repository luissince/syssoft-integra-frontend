...
class OrdenCompras extends CustomComponent {
  ...
  */
  ...
  loadData = async () => {
    ...
    if (
      ordenCompraLista &&
      ordenCompraLista.data &&
      ordenCompraLista.paginacion
    ) {
      ...
      this.refPaginacion.current.messagePaginacion = ordenCompraLista.paginacion.messagePaginacion;
    } else {
      await this.loadInit();
  ...
  fillTable = async (opcion = 0) => {
    ...
    const { success, data, message, type } = await listOrdenCompra(
      params,
