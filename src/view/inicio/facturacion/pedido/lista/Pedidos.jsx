 ...
 */
class Pedidos extends CustomComponent {
  /**
   *
   * Constructor
   */
  ...
  */
  ...
  loadingData = async () => {
    ...
    if (
      pedidoLista &&
      pedidoLista.data &&
      pedidoLista.paginacion
    ) {
      this.setState(pedidoLista.data);
      this.refPaginacion.current.upperPageBound = pedidoLista.paginacion.upperPageBound;
      this.refPaginacion.current.lowerPageBound = pedidoLista.paginacion.lowerPageBound;
      this.refPaginacion.current.isPrevBtnActive = pedidoLista.paginacion.isPrevBtnActive;
      this.refPaginacion.current.isNextBtnActive = pedidoLista.paginacion.isNextBtnActive;
      this.refPaginacion.current.pageBound = pedidoLista.paginacion.pageBound;
      this.refPaginacion.current.messagePaginacion = pedidoLista.paginacion.messagePaginacion;
  ...
  updateReduxState() {
    this.props.setListaPedidoData(this.state);
    this.props.setListaPedidoPaginacion({
  ...
  fillTable = async (opcion = 0) => {
    ...
    const { success, data, message, type } = await listPedido(params, this.abortControllerTable.signal);
 ...
 */
  ...
  handleEditar = (idPedido) => {
    this.props.history.push({
      pathname: `${this.props.location.pathname}/editar`,
      search: '?idPedido=' + idPedido,
    });
  };

  handleDetalle = (idPedido) => {
    this.props.history.push({
  ...
  handleAnular = async (id) => {
    const accept = await alertKit.question({
      title: 'Pedido',
      message: '¿Estás seguro de anular el pedido?',
      acceptButton: {
    ...
    if (accept) {
      ...
      const response = await anularPedido(params);
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  view === "tabla"
                    ? "grid grid-cols-[0.5fr_1fr_1.5fr_1.3fr_1.3fr_0.8fr_0.9fr_0.9fr_1.8fr] py-3 gap-x-3 items-center"
      ...
      <ContainerWrapper>
        ...
        <div className="flex flex-col gap-y-4 mb-4">
          <p className="text-gray-600 mt-1">
            Puedes ver las pedidos echas con diferentes filtros, por ejemplo: fechas de emisión, comprobante y estado.
          </p>
        </div>
        ...
        <div className={cn(
          ...
          <div className={cn(
            ...
            <div className={cn(
              ...
              <div className="grid grid-cols-[0.5fr_1fr_1.5fr_1.3fr_1.3fr_0.8fr_0.9fr_0.9fr_1.8fr] gap-x-3 py-3">
...
Pedidos.propTypes = {
  ...
  pedidoLista: PropTypes.shape({
    data: PropTypes.object,
    paginacion: PropTypes.object,
  }),
  setListaPedidoData: PropTypes.func,
  setListaPedidoPaginacion: PropTypes.func,
...
const mapStateToProps = (state) => {
  return {
    ...
    pedidoLista: state.predeterminado.pedidoLista,
  };
};

const mapDispatchToProps = { setListaPedidoData, setListaPedidoPaginacion };

const ConnectedPedidos = connect(mapStateToProps, mapDispatchToProps)(Pedidos);

export default ConnectedPedidos;
