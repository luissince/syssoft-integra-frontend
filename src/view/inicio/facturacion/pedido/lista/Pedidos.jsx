...
import { listPedido, cancelPedido } from '../../../../../network/rest/principal.network';
...
import { setListaPedidoData, setListaPedidoPaginacion } from '../../../../../redux/predeterminadoSlice';
...
class Pedidos extends CustomComponent {
  ...
  loadingData = async () => {
    if (this.props.pedidoLista && this.props.pedidoLista.data && this.props.pedidoLista.paginacion) {
      this.setState(this.props.pedidoLista.data)
      this.refPaginacion.current.upperPageBound = this.props.pedidoLista.paginacion.upperPageBound;
      this.refPaginacion.current.lowerPageBound = this.props.pedidoLista.paginacion.lowerPageBound;
      this.refPaginacion.current.isPrevBtnActive = this.props.pedidoLista.paginacion.isPrevBtnActive;
  ...
  fillTable = async (opcion, buscar = '') => {
    ...
    const response = await listPedido(params, this.abortControllerTable.signal);
  ...
  handleAnular = (id) => {
    alertDialog('Pedido', '¿Estás seguro de anular el pedido?', async (accept) => {
      if (accept) {
        const params = {
          idPedido: id,
        ...
        alertInfo("Pedido", "Procesando petición...")

        const response = await cancelPedido(params);

        if (response instanceof SuccessReponse) {
          alertSuccess("Pedido", response.data, async () => {
            await this.loadingInit()
          })
        }
        ...
        <Row>
          <Column formGroup={true}>
            <Button
              ...
              <i className="bi bi-file-plus"></i> Crear Pedido
            </Button>
            {' '}
            <Button
