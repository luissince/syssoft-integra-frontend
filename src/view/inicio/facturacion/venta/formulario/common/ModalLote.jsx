...
class ModalLote extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      producto: null,
      lotes: [],
      lotesSeleccionados: [], // Array de lotes seleccionados con sus cantidades
      cantidadTotal: 0, // Cantidad total seleccionada
      cantidadRequerida: '', // Cantidad que el usuario quiere vender
    }
  ...
  loadDatos = async (producto) => {
    ...
    if (response instanceof SuccessReponse) {
      ...
      // Ordenar lotes por fecha de vencimiento (próximos a vencer primero)
  ...
  handleAgregarCantidadLote = (lote, cantidad) => {
    ...
    if (cantidadNum <= 0 || cantidadNum > lote.cantidad) return;

    const lotesActualizados = [...lotesSeleccionados];
    const existingIndex = lotesActualizados.findIndex(item => item.idLote === lote.idLote);
    ...
    const lotesFiltrados = lotesActualizados.filter(item => item.cantidadSeleccionada > 0);

    // Calcular total
    const cantidadTotal = lotesFiltrados.reduce((total, item) => total + item.cantidadSeleccionada, 0);
  ...
  handleQuitarLote = (idLote) => {
    const { lotesSeleccionados } = this.state;
    const lotesActualizados = lotesSeleccionados.filter(item => item.idLote !== idLote);
    const cantidadTotal = lotesActualizados.reduce((total, item) => total + item.cantidadSeleccionada, 0);
  ...
  handleAutoCompletar = () => {
    ...
    const lotesDisponibles = lotes.filter(lote =>
      lote.cantidad > 0 && lote.diasRestantes > 0
    );
    ...
    const cantidadTotal = nuevosLotesSeleccionados.reduce((total, item) => total + item.cantidadSeleccionada, 0);
  ...
  handleOnSubmit = async () => {
    ...
    this.props.handleAdd(this.state.producto, lotesSeleccionados, cantidadTotal);
  }
  ...
  generarTablaLotes = () => {
    ...
    return lotes.map((lote, index) => {
      ...
      const loteSeleccionado = lotesSeleccionados.find(item => item.idLote === lote.idLote);
      const cantidadSeleccionada = loteSeleccionado ? loteSeleccionado.cantidadSeleccionada : 0;
      const isDisabled = lote.cantidad <= 0 || lote.diasRestantes <= 0;

      return (
        <TableRow
          key={index}
          className={`${cantidadSeleccionada > 0 ? 'table-primary' : ''} ${isDisabled ? 'table-secondary' : ''}`}>
          ...
          <TableCell>
            ...
            <ProgressBar
              value={rounded((lote.cantidad / this.state.producto?.cantidad) * 100, 0, 'number')}
            ...
            {!isDisabled && (
              <div className="d-flex align-items-center gap-2">
                <Input
                  ...
                  placeholder="0"
                />
                <span className="text-muted small">de {rounded(lote.cantidad)}</span>
              </div>
            )}
          </TableCell>
          ...
          <>
            ...
            <Row>
              <Column className="col-md-3" formGroup={true}>
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base m-0">Producto</CardTitle>
                  </CardHeader>
              ...
              <label>Opciones:</label>
                <div className="d-flex align-items-end gap-2"> 
                  ...
                  <Button
                    ...
                    onClick={() => this.setState({ lotesSeleccionados: [], cantidadTotal: 0 })}
                ...
                <Column formGroup={true}>
                  <Card className="border-success">
                    <CardHeader className="bg-success text-white">
                      <CardTitle className="text-base m-0">Resumen de Selección</CardTitle>
                    </CardHeader>
                    <CardBody>
                      <div className="d-flex justify-content-between align-items-center">
                        <span>
                          <strong>{lotesSeleccionados.length}</strong> lotes seleccionados
                        </span>
                        <span>
                          Total: <strong>{cantidadTotal}</strong> {producto?.medida || 'unid'}
                        </span>
                      </div>
                      <div className="mt-2">
                        {lotesSeleccionados.map((lote, index) => (
                          <small key={index} className="d-block text-muted">
                            {lote.codigoLote}: {lote.cantidadSeleccionada} {producto?.medida || 'unid'}
                          </small>
                        ))}
                      </div>
                    </CardBody>
                  ...
                  <Table className="table-bordered table-hover">
                    <TableHeader className="thead-light">
                      <TableRow>
                        ...
                        <TableHead width="20%" className="text-center">Cantidad a usar</TableHead>
                        <TableHead width="8%" className="text-center">Acción</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
