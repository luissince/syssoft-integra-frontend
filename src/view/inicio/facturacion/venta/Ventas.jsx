...
class Ventas extends CustomComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      add: statePrivilegio(
        this.props.token.userToken.menus[2].submenu[1].privilegio[0].estado,
      ),
      view: statePrivilegio(
        this.props.token.userToken.menus[2].submenu[1].privilegio[1].estado,
      ),
      remove: statePrivilegio(
        this.props.token.userToken.menus[2].submenu[1].privilegio[2].estado,
      ),
  ...
  handleAnular(idVenta) {
    alertDialog(
      'Venta',
      '¿Está seguro de que desea eliminar la venta? Esta operación no se puede deshacer.',
      async (value) => {
        if (value) {
          const params = {
            idVenta: idVenta,
      ...
      return (
        <tr key={index}>
          <td className={`text-center`}>
            {item.estado === 4 ? <del>{item.id}</del> : item.id}
          </td>
          <td>
            {item.estado === 4 ? (
              <>
          ...
          <td>
            {item.estado === 4 ? (
              ...
              <>
                {item.tipo === 1 ? <span>Contado</span> : <span>Crédito</span>}
              </>
            )}
          </td>
          <td>
