...
class VentaProceso extends CustomComponent {
    ...
    */
    ...
    async fetchComprobante(tipo) {
        ...
        const response = await comboComprobante(params, this.abortControllerView.signal);
    ...
    */
    ...
    handleFilterProducto = async (event) => {
        ...
        if (response instanceof SuccessReponse) {
            await this.setStateAsync({ filterProducto: false, productos: response.data, sarchProducto: false });
        }

        if (response instanceof ErrorResponse) {
            await this.setStateAsync({ filterProducto: false, productos: [], sarchProducto: false });
        }
    }
            ...
            if (value) {
                ...
                if (response instanceof ErrorResponse) {
                    if (response.getBody() !== "") {
                        ...
                        const body = response.getBody().map((item, index) => (
                            `<tr key={index}>
                                <td>${item.nombre}</td>
    ...
    handleStarProduct = (producto) => {
        ...
        this.setState((prevState) => {
            const updatedProductos = prevState.productos.map((item) =>
                item.idProducto === producto.idProducto ? { ...item, preferido: producto.preferido } : item
            );
