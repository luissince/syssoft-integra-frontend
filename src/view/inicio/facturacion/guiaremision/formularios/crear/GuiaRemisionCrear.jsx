...
import {
  ...
  getIdSucursal,
  getPreferidoPersona,
  getShippingGuideTraslado,
  getUbigeo,
  getShippingGuideVenta,
  filterVenta,
...
class GuiaRemisionCrear extends CustomComponent {
  ...
  */
  ...
  clearView = () => {
    this.setState(this.initial, async () => {
      await this.refVenta.current.restart();
      await this.refVehiculo.current.restart();
      await this.refConductor.current.restart();
  ...
  */
  ...
  handleSelectItemVehiculo = (value) => {
    this.refVehiculo.current.initialize(`${value.marca}-${value.numeroPlaca}`);
    this.setState({
      vehiculo: value,
      vehiculos: [],
    });
  ...
  handleClear = async () => {
    const accept = await alertKit.question({
      title: 'Guía de Remisión',
      message: '¿Está seguro de limpiar todo el contenido?',
    });
