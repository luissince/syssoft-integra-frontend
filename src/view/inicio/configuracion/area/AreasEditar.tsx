...
import { getIdArea, updateArea } from '@/network/rest/api-client';
...
class AreaEditar extends CustomComponent<Props, State> {
  ...
  async componentDidMount() {
    ...
    const idArea = new URLSearchParams(url).get("idArea");

    if (isText(idArea)) {
      this.loadingData(idArea);
    } else {
  ...
  async loadingData(id) {
    const area = await this.fetchObtenerArea(id);
    ...
    this.setState({
      ...
      estado: area.estado === 1 ? true : false,

      loading: false,
    });
  }

  async fetchObtenerArea(id) {
    const response = await getIdArea(id, this.abortController.signal);
...
const ConnectedAreaEditar = connect(
  mapStateToProps,
  null,
)(AreaEditar);

export default ConnectedAreaEditar;