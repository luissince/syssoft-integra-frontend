...
import {
    currentDate} from "../../../../helper/utils.helper";
import CustomComponent from "../../../../model/class/custom-component";
import SuccessReponse from '../../../../model/class/response';
import ErrorResponse from '../../../../model/class/error-response';
import { cancelAjuste, comboTipoAjuste, listAjuste } from '../../../../network/rest/principal.network';
...
class Ajuste extends CustomComponent {
    ...
    async loadingData() {
        const [tipoAjuste] = await Promise.all([
            await this.fetchComboTipoAjuste()
        ]);
    ...
    handleCancelar = (idAjuste) => {
        alertDialog("Ajuste", "¿Estás seguro de anular el ajuste?", async (acccept) => {
            if (acccept) {
    ...
    generarBody = () => {
        ...
        return this.state.lista.map((item, index) => {
            ...
            const iconTipoAjuste = item.ajuste === "INCREMENTO"
                                ...
                                >
                                    <option value="0">-- Selecciona --</option>
                                    {
                                        this.state.tipoAjuste.map((item, index) => (
                                            <option key={index} value={item.idTipoAjuste}>{item.nombre}</option>
                                        ))
                                    }
                                </select>
                            </div>
