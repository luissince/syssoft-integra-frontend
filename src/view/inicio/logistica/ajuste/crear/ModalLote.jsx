...
class ModalLote extends Component {
  ...
  generarTablaLotes = () => {
    ...
    return lotes.map((lote, index) => {
      ...
      const accountPorcent = (lote.cantidad / this.state.producto?.cantidad) * 100;

      let diferencia = 0;

      if (this.props.idTipoAjuste === INCREMENTO) {
        diferencia = Number(lote.cantidad) + getNumber(lote.cantidadAjustar);
      } else {
        diferencia = Number(lote.cantidad) - getNumber(lote.cantidadAjustar);
      }
...
ModalLote.propTypes = {
  ...
  idTipoAjuste: PropTypes.string.isRequired,
