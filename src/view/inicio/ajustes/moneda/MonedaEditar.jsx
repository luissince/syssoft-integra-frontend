...
import { editMoneda, getIdMoneda } from '../../../../network/rest/principal.network';
...
class MonedaEditar extends CustomComponent {
    ...
    async loadingData(idMoneda) {
        const [moneda] = await Promise.all([
            await this.fetchGetIdMoneda(idMoneda)
        ])
    ...
    handleEditar = () => {
        if (!isText(this.state.nombre)) {
            alertWarning("Moneda", "Ingres el nombre.", () => this.refTxtNombre.current.focus())
            return;
        }

        if (!isText(this.state.codIso)) {
            alertWarning("Moneda", "Ingres el código.", () => this.refTxtCodIso.current.focus())
            return;
        }
