...
async function generateLoteDeuda(req, sedeInfo, data) {
    try {
        ...
        ws.cell(6, 1, 6, 10, true).string(`REPORTE GENERAL DE VENTAS`).style(styleTitle);
        ws.cell(7, 1, 7, 10, true).string(`${req.query.porProyecto == "0" ? "PROYECTO: " + req.query.nombreProyecto : "TODOS LOS PROYECTOS"}`).style(styleTitle);

        const header = ["N°", "Cliente", "Propiedad", "Comprobante", "Cta Total", "Ctas Pagadas", "Ctas Pendiente", "Total", "Cobrado", "Por Cobrar"];
        ...
        data.map((item, index) => {
            ...
            ws.cell(rowY, 5).string(item.credito === 1 ? item.frecuencia : item.cuoTotal === 1 ? item.cuoTotal + " Cuota" : item.cuoTotal + " Cuotas").style(styleBody)
            ws.cell(rowY, 6).string(item.credito === 1 ? "-" : cuotaPagada === 1 ? cuotaPagada + " Cuota" : cuotaPagada + " Cuotas").style(styleBody)
            ws.cell(rowY, 7).string(item.credito === 1 ? item.frecuencia : item.numCuota === 1 ? item.numCuota + " Cuota" : item.numCuota + " Cuotas").style(styleBody)
